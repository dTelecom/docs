...
| `DTELECOM_API_HOST` | `https://<your-dtelecom-host>` | Twirp HTTP endpoint for `RoomServiceClient` (room/participant management) |

The `API_KEY` and `API_SECRET` are project-specific (from the cloud dashboard). The three `SOLANA_*` values are mainnet constants shared across all projects. `DTELECOM_API_HOST` is only needed if you use [Server APIs](/guides/server-api) to manage rooms or moderate participants.
