...
Use React hooks to get the list of participants in a room:

```tsx title="React"
import { useParticipants, useRemoteParticipants, useLocalParticipant } from '@dtelecom/components-react';
...
function ParticipantList() {
  ...
  return (
    <div>
      <p>You: {localParticipant.name || localParticipant.identity}</p>
      <p>{remoteParticipants.length} other(s) in the room</p>
      <ul>
        {participants.map((p) => (
...
These hooks must be used inside a `<LiveKitRoom>` component. They automatically update when participants join or leave.

## Getting a specific track

To access a specific track from a participant, use `participant.getTrack()` with a `Track.Source`:

```typescript
import { Track } from '@dtelecom/livekit-client';

// Get a participant's microphone track
const micPub = participant.getTrack(Track.Source.Microphone);
// Returns TrackPublication | undefined

if (micPub) {
  console.log('Track SID:', micPub.trackSid);
  console.log('Is muted:', micPub.isMuted);
}

// Other track sources:
const cameraPub = participant.getTrack(Track.Source.Camera);
const screenPub = participant.getTrack(Track.Source.ScreenShare);
```
