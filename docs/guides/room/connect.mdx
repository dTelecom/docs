...
The `url` is a WebSocket URL returned by `token.getWsUrl(clientIp)` on your server. Unlike centralized services, dTelecom does not use a fixed server URL — the URL is dynamically resolved from the Solana node registry based on the client's IP for optimal latency. See [Architecture](/guides/architecture) for details.
...
:::tip
`<LiveKitRoom>` automatically sets `data-lk-theme="default"` on its container, activating component styles. For components **outside** `<LiveKitRoom>` (like `<PreJoin>`), wrap them in `<div data-lk-theme="default">`. Available themes: `"default"`, `"huddle"`.
:::
